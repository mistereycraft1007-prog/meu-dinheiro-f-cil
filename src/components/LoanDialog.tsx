...
import { cloudDb, Loan, Transaction } from "@/lib/cloudDb";
...
export function LoanDialog({ open, onOpenChange, loan, onSuccess }: LoanDialogProps) {
  ...
  useEffect(() => {
    if (loan) {
      ...
      cloudDb.getTransactions(loan.id).then(setTransactions);
    } else {
      setFormData({
        person_name: "",
        loan_amount: "",
      ...
      if (loan) {
        if (loan.status !== "Pago" && loanData.status === "Pago") {
          await cloudDb.addTransaction({
            ...
            description: `Pagamento total - R$ ${Number(loan.amount_to_pay).toFixed(2)}`,
          });
        } else {
          await cloudDb.addTransaction({
            loan_id: loan.id,
        ...
        await cloudDb.updateLoan(loan.id, loanData);
        toast.success("Empréstimo atualizado com sucesso!");
      } else {
        await cloudDb.addLoan(loanData);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[525px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>{loan ? "Atualize as informações do empréstimo" : "Adicione um novo empréstimo ao sistema"}</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="person_name">Nome da Pessoa *</Label>
              <Input id="person_name" value={formData.person_name} onChange={(e) => setFormData({ ...formData, person_name: e.target.value })} placeholder="Digite o nome completo" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                ...
                <Input id="loan_date" type="date" value={formData.loan_date} onChange={(e) => setFormData({ ...formData, loan_date: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
