...
export function HaverDialog({ open, onOpenChange, loan, onSuccess }: HaverDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (haverAmount > Number(loan.amount_to_pay)) {
      toast.error("O valor do haver não pode ser maior que o valor a receber");
      return;
    }

    try {
      const newAmountToPay = Number(loan.amount_to_pay) - haverAmount;
      ...
      await cloudDb.updateLoan(loan.id, {
        ...
        amount_received: Number(loan.amount_received) + haverAmount,
      ...
      await cloudDb.addTransaction({
        ...
        type: "haver",
        amount: haverAmount,
        description: `Haver de R$ ${haverAmount.toFixed(2)} - A receber agora: R$ ${Math.max(newAmountToPay, 0).toFixed(2)} - Vencimento: ${newDueDate.toLocaleDateString("pt-BR")}`,
      });

      toast.success(`Haver de R$ ${haverAmount.toFixed(2)} registrado!`);
      setAmount("");
      onOpenChange(false);
      onSuccess();
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-[400px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {loan && (
              <>
                Devedor: <strong>{loan.person_name}</strong><br />
                Valor a receber atual: <strong>R$ {Number(loan.amount_to_pay).toFixed(2)}</strong>
              </>
            )}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Input id="haver_amount" type="number" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" autoFocus />
              {loan && amount && parseFloat(amount) > 0 && (
                <p className="text-sm text-muted-foreground">
                  Novo valor a receber: <strong>R$ {(Number(loan.amount_to_pay) - parseFloat(amount)).toFixed(2)}</strong>
                </p>
              )}
            </div>
          </div>
